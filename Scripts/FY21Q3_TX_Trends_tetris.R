...
# MUNGE -------------------------------------------------------------------
  ...
  df_tx <- df %>% 
    ...
    mutate(achv = cumulative / targets,
           ...
           gap = ifelse(fiscal_year == 2021, targets - cumulative, NA_real_))
...
nudge_space <- 0.125
  ...
  si_save("Graphics/FY21Q3_TX_trends_ou_gaps.svg", height = 4, width = 6.7, scale = 1.4)
  ...
  si_save("Graphics/FY21Q3_TX_CURR_ou_waffle.svg", height = 4, width = 4, scale = 1.25)
