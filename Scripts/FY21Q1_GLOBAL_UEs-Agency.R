...
df <- df %>% 
  mutate(agency = fct_inorder(agency))

# PLOT --------------------------------------------------------------------

df %>% 
  ggplot(aes(ue, fct_reorder(category, ue), fill = agency)) +
  geom_point(data = filter(df, agency == "CDC"), shape = 21, color = 'white', stroke = 1.4, size = 9) +
  geom_point(data = filter(df, agency == "USAID"), shape = 21, color = 'white', stroke = 1.4, size = 9) +
