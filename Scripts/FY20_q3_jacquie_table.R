...
df1 <- df %>% 
  filter(indicator %in% indc2,
         ...
         fundingagency != "Dedup") %>%
  select(operatingunit, fundingagency, psnu, mech_name, standardizeddisaggregate, otherdisaggregate, sex, trendsfine, indicator, fiscal_year, targets:cumulative) %>%
  group_by(operatingunit, fundingagency, psnu, mech_name, standardizeddisaggregate, otherdisaggregate, sex, trendsfine, fiscal_year, indicator) %>% 
...
df_vl <- df %>%
  ...
  mutate(indicator = ifelse(numeratordenom == "D", paste0(indicator, "_D"), indicator),
         ...
         indicator = case_when(indicator == "TX_CURR_N" ~ "TX_CURR",
                               TRUE ~ indicator)) %>% 
  select(operatingunit, fundingagency, psnu, mech_name, standardizeddisaggregate, sex, trendsfine, indicator, fiscal_year, starts_with("qtr")) %>%
  group_by(operatingunit, fundingagency, psnu, mech_name, standardizeddisaggregate, sex, trendsfine, fiscal_year, indicator) %>% 
...
df_all %>%
  ...
  summarise_if(is.numeric, ~ sum(., na.rm = TRUE)) %>%
  spread(indicator, val) %>%
  mutate(coverage = TX_PVLS_D/TX_CURR_lag2,
         suppression = TX_PVLS_N/TX_PVLS_D,
...
df_all %>%
  ...
  mutate(indicator = factor(indicator, ind_list),
         ...
         fundingagency = factor(fundingagency, c("USAID", "CDC"))) %>% 
  spread(period, val) %>% 
  write_csv(file.path(data_out, "drc_overview_tbl_q2.csv"))
...
df_all %>%
  ...
  mutate(indicator = factor(indicator, ind_list),
         ...
         fundingagency = factor(fundingagency, c("USAID", "CDC"))) %>% 
  spread(period, val) %>%
  write_csv(file.path(data_out, "drc_overview_tbl_peds_q2.csv"))
...
df_all %>%
  ...
  mutate(indicator = factor(indicator, ind_list),
         ...
         fundingagency = factor(fundingagency, c("USAID", "CDC"))) %>% 
  spread(period, val) %>% 
  write_csv(file.path(data_out, "bots_overview_tbl_q2.csv"))
...
df_all %>%
  ...
  mutate(indicator = factor(indicator, ind_list),
         ...
         fundingagency = factor(fundingagency, c("USAID", "CDC"))) %>% 
  spread(period, val) %>%
  write_csv(file.path(data_out, "bots_overview_tbl_peds_q2.csv"))

## RISE nigeria
df_all %>%
  filter(standardizeddisaggregate %in% c("Total Numerator", "Total Denominator"),
         operatingunit == "Nigeria",
         mech_name == "Reaching Impact, Saturation and Epidemic Control (RISE)",
         fundingagency %in% c("USAID", "HHS/CDC")) %>% 
  group_by(fundingagency, indicator, period) %>% 
  summarise_if(is.numeric, ~ sum(., na.rm = TRUE)) %>%
  spread(indicator, val) %>%
  ...
  mutate(indicator = factor(indicator, ind_list),
         ...
         fundingagency = factor(fundingagency, c("USAID", "CDC"))) %>% 
  spread(period, val) %>% 
  write_csv(file.path(data_out, "rise_ng_overview_tbl_q2.csv"))
...
df_all %>% filter(operatingunit == "Cote d'Ivoire") %>% 
  write_csv(file.path(data, "cdi_msd_q2_v1.csv"))
